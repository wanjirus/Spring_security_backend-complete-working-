package stan.inc.controllers.property;//package stan.security.spring_security.controllers.property;
...
import stan.inc.service.property.PropertyServiceInterface;
import stan.inc.service.property.TypeService;
...
//    }
    ...
    public ResponseEntity<Object> getPropertyById(@PathVariable(value = "propertyId") Long propertyId) throws ResourceNotFoundException {
        try {
            Property propertyDTO = propertyServiceInterface.findPropertyById(propertyId);
            return new ResponseEntity<>(propertyDTO, HttpStatus.OK);
        }catch (Exception e){
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
